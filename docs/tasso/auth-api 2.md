...
## 9. POST `/api/auth/staff/login`

Staff login step 1. Validates email and password for users with `admin` or `provider` role, then dispatches a 6-digit OTP to the supplied email address. **No auth cookies are issued yet** — cookies are only set after a successful OTP verification in step 2.

The OTP is stored in Redis with a 5-minute TTL. If an OTP was recently sent and has not expired, a second call to this endpoint will return `400` rather than overwriting the active OTP (prevents resend spam).
...
|---|---|
| `400` | Invalid payload (Zod) |
| `400` | OTP recently sent — wait for it to expire before requesting another |
| `401` | Invalid credentials, inactive account, or account is not `admin`/`provider` role |
| `401` | Admin authentication not configured server-side |
| `500` | Unexpected server error |

---

## 10. POST `/api/auth/staff/verify-otp`

Staff login step 2. Verifies the 6-digit OTP dispatched during step 1, creates a full auth session, and issues `apex_access_token` + `apex_refresh_token` cookies.

The OTP allows up to 5 incorrect attempts before being invalidated. After validation (correct or max-attempts reached), the OTP record is deleted from Redis.
...
Auth cookies are set on the response (same cookie config as regular login).
...
|---|---|
| `400` | Invalid payload (Zod) |
| `401` | OTP expired or not found — step 1 must be retried |
| `401` | Invalid OTP code |
| `401` | Too many incorrect attempts — OTP invalidated, step 1 must be retried |
| `401` | Account inactive or role changed since step 1 |
| `500` | Unexpected server error |

### Full Staff Login Flow

```
1. Staff user → POST /api/auth/staff/login  { email, password }
   ← { otpRequired: true }   (OTP dispatched to email, 5-minute TTL)

2. Staff user checks email for 6-digit code.

3. Staff user → POST /api/auth/staff/verify-otp  { email, code }
   ← { user: {...} }   (auth cookies issued)

4. All admin/provider-protected routes now work using the issued cookies.
```

---

## 11. GET `/api/admin/users` *(stub)*
